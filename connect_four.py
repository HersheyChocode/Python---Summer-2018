...
def turnX(board):
        xColumn= int(input(playerX+" you're X. What column do you want to play in?")) # Input of wanted column
        while xColumn>6 or xColumn<0:
                xColumn = int(input("Please enter a valid answer: "))
...
def turnO(board):
        oColumn= int(input(playerO+" you're O. What column do you want to play in?")) # Input of wanted column
        while oColumn>6 or oColumn<0:
                oColumn = int(input("Please enter a valid answer: "))
        for row in range(5,-1,-1): #For loop for row from 5 to 0
...
while True: #Repeats until broken
        if '. ' not in gameBoard:
            print("Wow! You guys Tied! Congratulations, and good game!")
            break
